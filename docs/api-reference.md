...
| `list_channels()` | Lists the currently active channels. |
| `list_subscribers(channel)` | Lists the subscribers of a channel. |
| `list_patterns()` | Lists the currently active patterns. |
| `list_pattern_subscribers(pattern)` | Lists the subscribers of a pattern. |
| **Persistence** | |
...
| `register_persistence_backend(name, backend)` | Registers a new persistence backend. |
| `set_default_persistence_backend(name)` | Sets the default persistence backend to use for `save` and `load`. |
| `snapshot()` | Creates a snapshot of the current dataset. |
| **Time Source** | |
| `set_time_source(time_source)` | Sets the time source for key expiry. |
| `get_time_source()` | Gets the current time source. |
| **Instance** | |
| `set_instance_name(name)` | Sets the name of the Gedis instance. Useful for debugging. |
| `get_instance_name()` | Gets the name of the Gedis instance. |